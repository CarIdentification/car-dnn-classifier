...
def tfrecords_input_fn(file_paths, batch_size=100, num_epochs=None, shuffle=False):
    def data_generate():
        ...
        dataset = dataset.shuffle(buffer_size=8144)  # 在训练的时候一般需要将输入数据进行顺序打乱提高训练的泛化性
...
def vgg_model_fn(features, labels, mode):
    ...
    dropout1 = tf.layers.dropout(
        inputs=dense1, rate=0.3, training=mode == tf.estimator.ModeKeys.TRAIN)  # 插入一个dropout层 丢弃30%节点

    dense2 = tf.layers.dense(inputs=dropout1, units=4096, activation=tf.nn.relu)  # 进行4096单元的全连接，激活函数为RELU
    dropout2 = tf.layers.dropout(
        inputs=dense2, rate=0.3, training=mode == tf.estimator.ModeKeys.TRAIN)  # 插入一个dropout层 丢弃30%节点

    # Logits Layer
    logits = tf.layers.dense(inputs=dropout2, units=196)  # 输出层  输出类型为四个方向   1:前方 2:侧面 3:前面+侧面 4:后面
    ...
    # learning_rate = tf.train.exponential_decay(0.001, tf.train.get_global_step(), 10, 0.0005)  # 计算出按步数递减的学习率
